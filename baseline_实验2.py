...
bayonet_data_csv_file = "C:/Users/rbppsuc403proMax/Desktop/基于频繁模式的车辆卡口数据预测/实验2/experiment_data/train_3.csv"
...
# test的路径
test_bayonet_csv_file = "C:/Users/rbppsuc403proMax/Desktop/基于频繁模式的车辆卡口数据预测/实验2/experiment_data/test_3.csv"
test_bayonet_data = np.loadtxt(test_bayonet_csv_file, dtype="str", delimiter=",", skiprows=1, usecols=[2, 3, 4])
...
for data in test_trajectory_list_list:
    if len(data) != 2:
        data = data[:2]
    tmp_tree = tree
    try:
...
# 此dfs遍历时记得加了几次就要减几次
def t_tree_dfs_prediction(tmp, node):
    ...
    if len(tmp) < len(data):
        if node.child:
            ...
            for next_node in node.child:
                ...
                tmp_node = next_node
                tmp = t_tree_dfs_prediction(tmp, tmp_node)
...
tmp = t_tree_dfs_prediction(tmp, node)
            ...
            for index, each_point in enumerate(each_list, 0):
                ...
                if count == len(data) + 1:
                    ...
                    for i in range(len(data) + 1):
                        ...
                        print(node.support)
                        node.support = each_list[i].support + get_list[index_get_list][i].support
                        print(node.support,node)
                        get_list[index_get_list][i] = node
