...
def main():
    ...
    # time.sleep(1000)
    return tree,trajectory_list_list
...
def load_tree(tree, data):
    ...
    #print(result_name,highest_score)
    return result_name
...
if __name__ == '__main__':
    ...
    tree,trajectory_list_list = main()
    print(len(trajectory_list_list))
    for i in trajectory_list_list:
        index += 1
        #print(all_dist)
        #print(index)
        sub = sub_lists(i)
        for j in sub:
            if tuple(j[:-1]) not in all_dist.keys():
                result = load_tree(tree, j[:-1])
                if all_dist == {}:
                    all_dist.update({tuple(j[:-1]):result})
                else:
                        all_dist.update({tuple(j[:-1]):result})
